...
export const EmptyState: React.FC<EmptyStateProps> = ({
  ...
  return (
    <Card className={`p-8 flex flex-col items-center justify-center text-center max-w-md mx-auto ${className}`} variant="glass">
      {icon && (
        <div className="p-4 bg-slate-800/40 rounded-full border border-slate-700/40 text-slate-400 mb-4">
          {icon}
        </div>
      )}
      <h5 className="text-lg font-bold text-slate-200 tracking-tight">{title}</h5>
