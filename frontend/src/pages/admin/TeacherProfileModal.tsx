        ...
        <div className="flex justify-between items-start mb-6 shrink-0">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-slate-600 font-mono mt-1">{teacher.email}</p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 overflow-y-auto pr-2 pb-4 flex-1">
          ...
          <div className="space-y-6">
            <Card className="p-5" variant="glass">
              <h4 className="font-bold text-slate-800 text-sm mb-4">Teaching Assignment Stats</h4>
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-indigo-500/10 border border-indigo-500/20 p-3 rounded-xl">
                  <p className="text-xs text-slate-600 uppercase tracking-widest font-semibold mb-1">Assigned Classes</p>
                  <p className="text-xl font-extrabold text-indigo-400">{teacher.classIds?.length || 0}</p>
                </div>
                <div className="bg-blue-500/10 border border-blue-500/20 p-3 rounded-xl">
                  <p className="text-xs text-slate-600 uppercase tracking-widest font-semibold mb-1">Assigned Subjects</p>
                  <p className="text-xl font-extrabold text-blue-400">{teacher.subjects?.length || 0}</p>
                </div>
                <div className="bg-amber-500/10 border border-amber-500/20 p-3 rounded-xl">
                  <p className="text-xs text-slate-600 uppercase tracking-widest font-semibold mb-1">Ungraded Subs</p>
                  <p className="text-xl font-extrabold text-amber-400">{pendingGrades}</p>
                </div>
                <div className="bg-rose-500/10 border border-rose-500/20 p-3 rounded-xl">
                  <p className="text-xs text-slate-600 uppercase tracking-widest font-semibold mb-1">Pending AOIs</p>
            ...
            <Card className="p-5" variant="glass">
              <h4 className="font-bold text-slate-800 text-sm mb-4">Taught Streams</h4>
          ...
          <div className="space-y-6">
            <Card className="p-5" variant="glass">
              <h4 className="font-bold text-slate-800 text-sm mb-4">Subjects Handled</h4>
              <div className="flex flex-wrap gap-2">
            ...
            <Card className="p-5" variant="glass">
              <h4 className="font-bold text-slate-800 text-sm mb-4">Recent Activities Configured</h4>
              <div className="space-y-3">
                {myAois.slice(0, 5).map(aoi => (
                  <div key={aoi.id} className="p-3 bg-white/40 rounded-xl border border-black/5 flex justify-between items-center">
                    <div>
                      <p className="text-xs font-bold text-slate-700">{aoi.title}</p>
